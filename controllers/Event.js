...
const { sendFcmMessageToUser } = require('../services/pushNotification');

exports.getEvent = (req, res, next) => {
  const { category, musicianId, customerId, dateEvent, status } = req.query;
  ...
  if (dateEvent) {
    query = {
      dateEvent: {
        $gte: moment(dateEvent)
          .startOf('day')
          .toDate(),
        $lte: moment(dateEvent)
          .endOf('day')
          .toDate()
      },
      status
  ...
  if (dateEvent && musicianId) {
    query = {
      musicianId: mongoose.mongo.ObjectId(musicianId),
      dateEvent: {
        $gte: moment(dateEvent)
          .startOf('day')
          .toDate(),
  ...
  if (dateEvent && customerId) {
    query = {
      customerId: mongoose.mongo.ObjectId(customerId),
      dateEvent: {
        $gte: moment(dateEvent)
          .startOf('day')
          .toDate(),
...
exports.addEvent = (req, res, next) => {
  ...
  const { category, musicianId, dateEvent, duration, location } = req.body;
